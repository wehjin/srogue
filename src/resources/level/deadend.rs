...
pub fn make_deadend(sector: Sector, do_recurse: bool, current_level: usize, spaces: &mut [SectorSpace; 9], map: &mut LevelMap) -> Vec<Sector> {
...
fn make_recursive(sector: Sector, spot: LevelSpot, current_level: usize, spaces: &mut [SectorSpace; 9], map: &mut LevelMap) -> Vec<Sector> {
	spaces[sector as usize].ty = RoomType::DeadEnd;
	map.put_feature_at_spot(spot, Feature::Tunnel);
...
fn get_targets(sector: Sector, spaces: &[SectorSpace; 9]) -> Vec<Target> {
...
impl Target {
	pub fn try_new(sector: Sector, neighbor: SectorNeighbor, spaces: &[SectorSpace; 9]) -> Option<Self> {
